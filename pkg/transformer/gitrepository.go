...
func (g *GitRepository) TransformGitRepository(module *deployv1alpha1.Module) mf.Transformer {
	return func(u *unstructured.Unstructured) error {
		...
		if u.GetKind() == "GitRepository" && git != nil {
			...
			switch git.Ref.Type {
			case "branch":
				gitrepo.Spec.Reference = &sourcev1.GitRepositoryRef{Branch: git.Ref.Value}
			case "tag":
				gitrepo.Spec.Reference = &sourcev1.GitRepositoryRef{Tag: git.Ref.Value}
			case "commit":
				gitrepo.Spec.Reference = &sourcev1.GitRepositoryRef{Commit: git.Ref.Value}
			case "semver":
				gitrepo.Spec.Reference = &sourcev1.GitRepositoryRef{SemVer: git.Ref.Value}
			}
